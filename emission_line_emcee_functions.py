...
class MCMC_functions():
	def __init__(self, model, model_bounds):
		self.model = model
	...
	def lnprior(self, theta):
		...
		# 	return -np.inf
		# else:
		if ( 0.0005 <= theta[0] <= 0.002) and (3.5 <= theta[1] <= 4.5) and (0.0 <= theta[2] <= 20):
			return 0.0
		return -np.inf
	...
	def run_emcee(self, lnprob, ndim, nwalkers, nchains, thetaGuess, args):
